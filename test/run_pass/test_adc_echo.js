                        ...
                        if (err) {
                            ...
                            gpioPins[GPIO_ADC_MODE_1].write(0, function (err) {
                                if (err) {
                                    ...
                                    gpioPins[GPIO_ADC_MODE_2].write(1, function(err) {
                                        if (err) {
                                            assert(!err, 'adc: gpio error ' + err);
                                        } else {
                                            medianRead(adc0, 10, function (err, value) {
                                                console.log('adc: 3rd read ' + value);
                                                if (err) {
                                                    assert(!err, 'adc: 3rd read error ' + err);
                                                } else if (value > 1200) {
                                                    assert(value, 'adc: proper value was returned, adc ok');
                                                } else {
                                                    assert(false, 'adc: value for mode 2 was wrong ' + value);
                                                }
                                            });
                                        }
                                    });
                            ...
                            assert(false, 'adc: value for mode 1 was wrong ' + value);
                        }
                    });
                }
            })
