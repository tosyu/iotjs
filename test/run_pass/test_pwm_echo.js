...
function runTest() {
    ...
    gpioPins[GPIO_PWM_ON_PIN].write(1, function (err) {
        if (err) {
            console.error('pwm error: failed to start adc mode on echo service');
        } else {
            ...
            setTimeout(function () { // wait for the device to respond
                ...
                gpioPins[GPIO_MODE_FULL_PIN].read(function (err, full) {
                    if (err) {
                        console.error('pwm error: could not read mode signal', err);
                    } else {
                        console.log('pwm: device responded with', full);
                        assert(full, 'pwm: echo service requested full duty cycle');
                        ...
                        setTimeout(function () {
                            ...
                            gpioPins[GPIO_PWM_OK_PIN].read(function (err, ok) {
                                if (err) {
                                    console.error('pwm error: could not read response from echo service', err);
                                } else {
                                    console.log('pwm: device responded with', ok);
                                    assert(ok, 'pwm: echo service confirmed PWM working');
                                }
                            });
                        }, 1000);
                    }
...
pwm0 = pwm.open({
    ...
    if (err) {
        ...
        [GPIO_PWM_ON_PIN, GPIO_MODE_FULL_PIN, GPIO_PWM_OK_PIN].forEach(function (gpioPin) {
            gpioPins[gpioPin] = gpio.open({
                pin: gpioPin,
                direction: gpioPin === GPIO_PWM_ON_PIN ? gpio.DIRECTION.OUTPUT : gpio.DIRECTION.INPUT,
                mode: gpio.MODE.NONE
            }, gpioOpenCallback);
        });
    }
